...
inline void RCS_USART_Send_Char( USART_TypeDef *_USARTx, uint8_t _character)
{
...
inline void RCS_USART_Send_Str(USART_TypeDef *_USARTx, uint8_t _TxBuffer[])
{
	uint8_t i = 0;
	while (USART_GetFlagStatus(_USARTx, USART_FLAG_TC) == RESET);
	for (; _TxBuffer[i] != '\0'; i++)
...
//@param:length  数据长度
inline void RCS_USART_Send_Data(USART_TypeDef *_USARTx, uint8_t *address, uint8_t length)
{
	uint8_t i = 0;
	while (USART_GetFlagStatus(_USARTx, USART_FLAG_TC) == RESET);
	for (; i < length ; i++)
...
//@param:_USARTx 已经绑定的USART串口
//@retval: uint16位的字符
inline uint16_t RCS_USART_Accept_Char(USART_TypeDef *_USARTx)
{
