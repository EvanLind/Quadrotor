...
typedef	struct Triaxial{
		float pitch;
		float roll;
		float yaw;
		uint8_t err_flag;			//错误标志位   0 -- 获取数据成功   1 --- 获取数据失败
}Triaxial;
...
/*
	调用MPU6050这个模块的时候，只要首先滴啊用初始化函数
		I2C_Configuration();			//硬件IIC初始化函数
		delay_nms(30);
		DMP_MPU6050_Init();			//6050DMP初始化
		
		最后只要调用DMP_MPU6050_GetData()   就能够得出三轴的角度了
*/
#endif
