#ifndef _DMP_DRIVER_H_
#define _DMP_DRIVER_H_
...
/* Set up functions. */			//启动函数
int dmp_load_motion_driver_firmware(void);			//**开启DMP函数
...
/* Android orientation functions. */		//orientation--方向；定向；适应
int dmp_register_android_orient_cb(void (*func)(unsigned char));

/* LP quaternion functions. */		//LP四元数函数
int dmp_enable_lp_quat(unsigned char enable);
int dmp_enable_6x_lp_quat(unsigned char enable);

/* Pedometer functions. */		//计步器功能
int dmp_get_pedometer_step_count(unsigned long *count);
int dmp_set_pedometer_step_count(unsigned long count);
int dmp_get_pedometer_walk_time(unsigned long *time);
int dmp_set_pedometer_walk_time(unsigned long time);

/* DMP gyro calibration functions. */ //DMP陀螺仪标定函数
int dmp_enable_gyro_cal(unsigned char enable);

/* Read function. This function should be called whenever the MPU interrupt is
 * detected.			//读函数，这个函数应该被叫做 MPU随时中断检测
 */
